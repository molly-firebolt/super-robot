...
def create_card_entry(parent_path, cardfile, line_prefix):
    card_id = path.splitext(cardfile)[0]
    card_path = os.path.relpath(cardfile, parent_path)
    #.lstrip(parent_path)
    #card_id = re.sub(r'[^a-zA-Z0-9]','_', card_id)
    return f'{line_prefix}  - ID: "{card_path}"\n' \
...
root_dir = sys.argv[1] if len(sys.argv)>1 else input('Directory to scan for markdown files (enter "./" for current directory): ')
#.strip()
